...
+ (UIImage*)numberPlateWithSobelFromCarImage:(UIImage*)image imageName:(NSString *)name {
    ...
    RotatedRect minRect = getOnePossiblePlateRegion(rects,0.5);
    
    if(!(minRect.center.x==0&&minRect.center.y==0&&minRect.size.width==0&&minRect.size.height==0)) {

        Mat mask;
        mask = detectRegions.getFloodFillMask(input_img, result, minRect);
        watchTestImg = [UIImage imageWithCVMat:mask];
...
RotatedRect getOnePossiblePlateRegion(vector<RotatedRect> rects, float error) {
    ...
    if (newRects.size()>1) {
        getOnePossiblePlateRegion(newRects,(error-0.05));
    }
    else if (newRects.size()==0) {
        rect = RotatedRect(cv::Point(0,0), cv::Size(0,0), 0);
    }
    else {
        rect = newRects[0];
    }
